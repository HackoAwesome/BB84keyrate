...
# -----------------------
# Evaluating using the third Renyi Entropy Formula
# -----------------------
...
def hterm3(n, aldelt, gamma, qberthresh, n_p):
    """
    Function computes the worst-case entropy-related term appearing in the finite-sized keyrate bound for EB-BB84 by carrying out convex optimisation to return the optimal value of the convex security bound from Eqn 145, along with the optimiser-chosen parameters that realise the worst-case eavesdropping scenario.
    ...
    for pi in p_grid:
        ...
        t1 = (pi / 2) ** (1 / alpha_p)
        t2 = (1 - 3 * pi / 2) ** (1 / alpha_p)

        C = t1 + t2

        K = 2 ** (((1 - alpha_p) * alpha) / alpha_p + 1)
        ...
        C_diff = (
        (1 / (2 * alpha_p)) * (pi / 2) ** (1 / alpha_p - 1)
        - (3 / (2 * alpha_p)) * (1 - 3 * pi / 2) ** (1 / alpha_p - 1)
        )
        ...
        A_diff = (
        2 * (alpha / alpha_p) * pi ** (alpha / alpha_p - 1)
        + K
        * alpha
        * C ** (alpha - 1)
        * C_diff
        ...
        B_diff = (
        (3 / alpha_p) * pi ** (1 / alpha_p - 1)
        - (3 / (2 * alpha_p))
        * 2 ** (1 / alpha_p)
        * (1 - 3 * pi / 2) ** (1 / alpha_p - 1)
        )
        ...
        df = (
        A_diff / ((1 - alpha) * A * np.log(2))
        - B_diff / (B * np.log(2))
        )

        f_pi = (1 - gamma - delta) * f

        df_dp = (1 - gamma - delta) * df
...
# Key rate function
# -----------------------
def keyrate3(aldelt, gamma, n, qberthresh, esound, n_p):

    sol_val, sol, status = hterm3(n, aldelt, gamma, qberthresh, n_p)
...
def rate_sixstate3(n, qberthresh, esound, n_p, n_aldelt, n_gamma):
    """
    Function computes the finite-size secure key rate for the entanglement-based BB84 protocol.
    It combines the worst-case entropy bound derived from 'htermBB84' with classical error-correction leakage and finite-sized security penalities to produce the final secure keyrate per signal as per Eqn 145 of arXiv:2405.05912.
    ...
    keyrate_partial = partial(keyrate3, n=n, qberthresh=qberthresh, esound=esound, n_p=n_p)
